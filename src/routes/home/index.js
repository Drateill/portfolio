import { h } from "preact";
import style from "./style.scss";

const Home = () => (
  <main>
    <h1><span>m</span>ylène <span>v</span>andaële</h1>
  
    <h2>Développeuse front-end</h2>

    <section class={style.presentation}>
      <div class={style["presentation__photo-container"]}>
        <div class={style["presentation__photo-border"]}>
          <img class={style.presentation__photo} src="../assets/img/photo-nb.jpg" />
        </div>
      </div>

      <p class={style.presentation__text}>lorem dedje de fd ed efe reg r gjjkrfjgksjkefd rgs srgkj esrg rtegre</p>
    </section>

    <div class={style["buttons-container"]}>
      <button type="button" class={style.button}>
        <span>Me contacter</span>
        <svg class={style["button-icon"]}
          xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M464 64H48C21.49 64 0 85.49 0 112v288c0 26.51 21.49 48 48 48h416c26.51 0 48-21.49 48-48V112c0-26.51-21.49-48-48-48zm0 48v40.805c-22.422 18.259-58.168 46.651-134.587 106.49-16.841 13.247-50.201 45.072-73.413 44.701-23.208.375-56.579-31.459-73.413-44.701C106.18 199.465 70.425 171.067 48 152.805V112h416zM48 400V214.398c22.914 18.251 55.409 43.862 104.938 82.646 21.857 17.205 60.134 55.186 103.062 54.955 42.717.231 80.509-37.199 103.053-54.947 49.528-38.783 82.032-64.401 104.947-82.653V400H48z"/>
        </svg>
      </button>

      <button type="button" class={style.button}>
        <span>Page suivante</span>
        <svg class={style["button-icon"]}
          xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M190.5 66.9l22.2-22.2c9.4-9.4 24.6-9.4 33.9 0L441 239c9.4 9.4 9.4 24.6 0 33.9L246.6 467.3c-9.4 9.4-24.6 9.4-33.9 0l-22.2-22.2c-9.5-9.5-9.3-25 .4-34.3L311.4 296H24c-13.3 0-24-10.7-24-24v-32c0-13.3 10.7-24 24-24h287.4L190.9 101.2c-9.8-9.3-10-24.8-.4-34.3z"/>
        </svg>
      </button>
    </div>
  </main>
);

export default Home;
